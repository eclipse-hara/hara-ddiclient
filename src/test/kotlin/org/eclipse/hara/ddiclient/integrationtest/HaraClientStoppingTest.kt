...
class HaraClientStoppingTest : AbstractTest() {
    ...
    fun haraClientShouldStopPollingAfterBeingStoppedInDownloadingState() {
        val controllerId =
            "haraClientShouldStopPollingAfterBeingStoppedInDownloadingState_${System.currentTimeMillis()}"
        logCurrentFunctionName()
        stoppingHaraClientWhileInDownloadingStateTestTemplate(
            controllerId,
            TestUtils.OS_DISTRIBUTION_ID
        )
    }
    ...
    fun haraClientShouldStopPollingAfterBeingStoppedInDownloadingStateForMultipleArtifacts() {
        val controllerId =
            "haraClientShouldStopPollingAfterBeingStoppedInDownloadingStateForMultipleArtifacts_${System.currentTimeMillis()}"
        logCurrentFunctionName()
        stoppingHaraClientWhileInDownloadingStateTestTemplate(
            controllerId,
    ...
    fun haraClientShouldStopPollingAfterBeingStoppedInUpdatingState() {
        val controllerId =
            "haraClientShouldStopPollingAfterBeingStoppedInUpdatingState_${System.currentTimeMillis()}"
        logCurrentFunctionName()
        runTest { testJob ->
    ...
    fun haraClientShouldPollAfterRestartedInDownloadingState() {
        val controllerId =
            "haraClientShouldPollAfterRestartedInDownloadingState_${System.currentTimeMillis()}"
        logCurrentFunctionName()
        runTest { testJob ->

            setPollingTime("00:00:10")
            reCreateTestTargetOnServer(controllerId)
            assignHeavyOTAUpdateToTheTarget(controllerId, TestUtils.OS_DISTRIBUTION_ID)
            ...
            listenToMessages(expectedMessageChannel, testClient) { msg, _ ->
                if (expectedMessage.isNotEmpty()) {
                    ...
                    testClient = createClient(controllerId, expectedMessageChannel)
                    expectedMessage.add(
    ...
    private fun stoppingHaraClientWhileInDownloadingStateTestTemplate(
        controllerId: String,
        distributionId: Int
    ) {
        runTest { testJob ->
            ...
            haraClientStopTestTemplate(
                ...
                onMessageReceive = { msg, testClient ->
    ...
    private suspend fun haraClientStopTestTemplate(
        ...
        reCreateTestTargetOnServer(controllerId)
        assignHeavyOTAUpdateToTheTarget(controllerId, distributionId)
    ...
    private suspend fun assignHeavyOTAUpdateToTheTarget(
        ...
        assignDistributionToTheTarget(controllerId, distribution)
    }
